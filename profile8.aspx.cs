...
{
    public partial class profile8 : System.Web.UI.Page
    {
        ...
        protected void loadNextSongsList(object sender, EventArgs e)
        {
            Control spanNext = FindControl("spanNext");
            ...
            sb.Append("<script language='javascript' type='text/javascript'>document.getElementById('spanNext').style.display = 'none';document.getElementById('loading').style.display = 'block'; ");
...
"document.getElementsByTagName('head')[0].appendChild(fileref);       $.when(" +
      ...
      "})" +
  ").done(function () {" +
  "    (function ($) {console.log(3433);" +
          "var settings = { 'compact_list': '0', 'color1': 'a30283', 'ID': 80, 'nonce': '08187fd71d', 'url': '', 'moduleUrl': '' };" +
          "$('#GmediaGallery_80').gmWaveSurfer(settings);" +
      "})(jQuery);" +
      "setTimeout(function () {document.getElementById('spanNext').style.display = 'block';document.getElementById('loading').style.display = 'none'; }, 8000);" +
  "});");
            sb.Append("}");
            sb.Append("</script>");
